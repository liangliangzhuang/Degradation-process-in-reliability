...
假设$Y(t) \sim \operatorname{IG}\left( \mu \Lambda(t)， \lambda \Lambda^2(t)\right)$。根据增量独立性和服从IG分布，模拟数据产生。

为方便起见，假设$\mu = 1$、$\Lambda(t) = \eta t$，$\eta = 1$。之后再加上其他线性表示方式(这里主要复现已有结果)。比如：$\Lambda(t) = t^\alpha$、$\Lambda(t) = \exp(t)$、$\Lambda(t) = \log(t)$。

```{r}
# 数据模拟产生
sim_dat = function(group = 6, t = 1:200, para = c(1,0.1), fun_nonlinear = "linear", fun_par = c(1)){
  if (fun_nonlinear == "linear"){
    Lam = t*fun_par[1]
  }
  # para 指 mu、lambda
  ...
  dat_unit = numeric()
  for(i in 1:group){
    for(j in 1:epoch) dat_unit[j] = rinvgauss(1, mean=para[1]*delta_t[j], shape=delta_t[j]^2*para[2]) # shape = 1/ dispersion, 我们这是shape
...
模拟产生数据，令$\mu =1,\lambda = 3, eta = 1$

```{r}
group = 6
t = 1:5
para = c(1,3,1) # mu = 1, lambda = 3, eta = 1
dat = sim_dat(group = group, t = t, para = para,
              fun_nonlinear = "linear", fun_par = para[3]) 
...
构建函数`IG_mle`，其中par中表示$\eta$和$\lambda$。

```{r}
IG_mle = function(par = c(1,3),data = dat){
  # 输入：eta,lambda 数据
  # 输出：负对数似然函数
...
mle_re = optim(par=c(1,3), fn = IG_mle,

data = dat, method = "BFGS", hessian = TRUE)

\# mle_re

mle_par = mle_re\$par \# 1，3
...
```{r}
mle_summary = round(cbind(mle_low,mle_re$par,mle_up,c(para[3],para[2])),3)
...
为了得到产品可靠度的估计, 只要将 $\hat{\eta} 、 \hat{\lambda}^2$ 代人时间 $t$ 的可靠度的表达式（1-F(t)）, 即可得到可靠度的点估计为 $$
...
```{r,collapse=TRUE}
## 可靠度计算
IG_R = function(t = 1, threshold = 6, par = mle_par){
...
plot(IG_R(t = 1:5, threshold = 5,par = mle_par),type = "l",ylab = 'Reliability',xlab = "time")
lines(IG_R(t = 1:5, threshold = 5,par = mle_up))
lines(IG_R(t = 1:5, threshold = 5,par = mle_low))
```

```{r}
IG_R(t = 1, threshold = 150,par = mle_par)
```
