...
Let $Y(t)$ be the degradation characteristic of the product at time $t$. Suppose that the degradation path $Y(t)$ can be modeled by an IG process with mean function $\Lambda(t)$ and the scale parameter $\lambda$. Thus, $Y(t)$ have the following properties:
- $Y(0)=0$ with probability one;
- For $t>s>u, Y(t)-Y(s) \geq 0, Y(s)-Y(u) \geq 0, Y(t)-Y(s)$ and $Y(s)-Y(u)$ are independent;
- For $t>s \geq 0$, each increment $Y(t)-Y(s)$ has an IG distribution $I G\left(\Lambda(t)-\Lambda(s), \lambda(\Lambda(t)-\Lambda(s))^2\right)$, where $\Lambda(t)$ is a monotonic increasing function, and $\Lambda(0)=0$.
The probability density function (pdf) of an IG distribution $I G(a, b)$ is
$$
...
\end{aligned}
$$
where $\phi(\cdot)$ is pdf of standard normal distribution.
If $t \rightarrow \infty$, then $\lambda \Lambda(t) \rightarrow \infty$ and $Y(t)$ follows an asymptotic normal distribution with mean $\Lambda(t)$, and variance $\frac{\Lambda(t)}{\lambda}$. Therefore, the cdf of lifetime $T$ is asymptotically
$$
...
\end{array}\right],
$$
where $\tau=\sum_{i=1}^n \sum_{j=1}^{m_i} \Delta t_{i j}$ and $N=\sum_{i=1}^n m_i$.
