...
如果产品的性能退化过程是一元 Wiener 过程，对应的失效阈值设定为$l$。则**可靠度**的表达式为 $$
...
假设其运行到时刻 $\tau$ 仍末失效, 且当前性能退化量为 $x_\tau\left(x_\tau<l\right)$, 则产品的**剩余寿命** $T_1$ 可以表示为 $$
...
数据模拟产生函数中，假设para为（$\mu,\sigma$）。这里假设$\mu = 2,\sigma = 2$。利用维纳过程的增量平稳性，并服从正态分布进行模拟产生数据。
...
为了得到产品可靠度的估计, 只要将 $\hat{\mu} 、 \hat{\sigma}^2$ 代人任务时间 $t$ 的可靠度的表达式, 即可得到可靠度的点估计。
...
R_data = data.frame(cbind(1:150,Wiener_R(t = 1:150, threshold = 150,par = mle_low),Wiener_R(t = 1:150, threshold = 150,par = mle_par),Wiener_R(t = 1:150, threshold = 150,par = mle_up)))
colnames(R_data) = c("Time","Low","Mean","Up") 
R_data %>% pivot_longer(cols = `Low`:`Up`,
                        names_to = "Class",
                        values_to = "Value") -> R_data
ggplot(R_data) +
  geom_line(aes(Time,Value, color = as.factor(Class)),linetype = 1) + 
  scale_color_viridis(discrete = T,name = "Type") + ylab("Reliability") 
```

根据前面所提的计算公式，得到RUL密度函数。

```{r}
# RUL密度函数
RUL_density = function(t = 1:10, cur_time = 50, threshold = 150, par = mle_par){
  RUL = (threshold - cur_time)/(sqrt(2 * pi * mle_par[2]^2 * t^3)) * exp(-((threshold - cur_time - mle_par[1]*t)^2)/(2*mle_par[2]^2 * t)) 
  return(RUL)
}
```

给一个简单例子，当前时刻为10，计算对应的RUL密度函数
```{r}
rul_den = RUL_density(t = 1:100, cur_time = 100, threshold = 150, par = mle_par)
plot(rul_den,type = 'l')
```

计算未来某些时刻（`fut_time`）的RUL，并绘制3D图形。

```{r}
#
fut_time = c(50,55,60,65,70,75,80)
time_epoch = 1:100
rul_den = matrix(NA,length(time_epoch),length(fut_time))
for(i in 1:length(fut_time)){
  rul_den[,i] = RUL_density(t = time_epoch, cur_time = fut_time[i], threshold = 150, par = mle_par)
}
rul_den
```
...
# 真实数据
true_rul_den = matrix(NA,length(time_epoch),length(fut_time))
for(i in 1:length(fut_time)){
  true_rul_den[,i] = RUL_density(t = time_epoch, cur_time = fut_time[i], threshold = 150, par = par)
}
real_RUL = apply(true_rul_den,2,which.max)
...
legend("top", legend = c("RUL distribution","Point estimation","Real value"),
       pch=c(NA,16,5), col = col[1:3],lty = c(1,NA,NA)) #,inset = .05
# 7.8,6.4
}
```
...
He, L., et al. (2016). "Objective Bayesian analysis of degradation model with respect to a Wiener process." Journal of Systems Science and Complexity 29(6): 1737-1751.


- 线性维纳+CSADT

Guan, Q., et al. (2016). "Objective Bayesian analysis accelerated degradation test based on Wiener process models." Applied Mathematical Modelling 40(4): 2743-2755.

- 线性IG+CSADT

He, D., et al. (2018). "Objective Bayesian analysis for the accelerated degradation model based on the inverse Gaussian process." Applied Mathematical Modelling 61: 341-350.

- 线性ED过程

Yan, W. A., et al. (2021). "Objective Bayesian Estimation for Tweedie Exponential Dispersion Process." Mathematics 9(21).
...
这里给出不同文献创建的模型和对应的参数估计方法

#### 3.3.1 二阶段似然估计方法

考虑漂移参数和扩散参数服从正态 - 逆 Gamma 分布的情形, 利用 $n$ 个样品的退化 试验数据对随机效应的分布参数 $\nu 、 \eta 、 \alpha 、 \beta$ 进行估计。这里采用两阶段法, 即首先估计个体产品的固定效应退化过程的参数, 再利用估计的参数值来得到随机效应分布参数 的估计。
...
基于随机效应的维纳过程，其中$Y(t) = \Theta t + \sigma_w W(t)$, $\Theta \sim N(\mu,\sigma^2_\mu)$。所以需要估计的参数为：$\theta = (\mu,\sigma^2_\mu,\sigma^2_\omega)$。

###### 先验确定

The Fisher information matrix of the parameter $\theta = (\mu,\sigma^2_\mu,\sigma^2_\omega)$ is given by $$
H(\theta)=\left(\begin{array}{ccc}
\frac{n t_m}{\sigma_\omega^2+t_m \sigma_\mu^2} & 0 & 0 \\
